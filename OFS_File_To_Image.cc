...
#define WIDTH_101H 640
#define HEIGHT_101H 480
...
/* "cat temp.ofs | netcat 127.0.0.1 [port]" is how to pipe file data into netcat for downloading external files */
...
uint8_t* ReadFileBMP(const char* filename, uint16_t* retWidth, uint16_t* retHeight, bool vesa) {
	...
	if (vesa == false && width > WIDTH_13H || height > HEIGHT_13H) {
	
		cerr << "BMP image is too large for 320x200 VGA resolution." << endl;
		return nullptr;
	}
	if (vesa == true && width > WIDTH_101H || height > HEIGHT_101H) {
	
		cerr << "BMP image is too large for 640x480 VESA resolution." << endl;
		return nullptr;
	}
...
int main(int argc, char* argv[]) {
	...
	if (argc < 2) {
		...
		cerr << "Please pass the file you want written." << endl;
		return -1;
	}
	const char* exfile = argv[1];
	...
	//this contains formatted osakaOS image data
	...
	if (findStr.find(".bmp") != string::npos) {
		...
		uint8_t* tmpBMP = ReadFileBMP(exfile, &width, &height, false);
	...
	//we will now create a new file with osakaOS data
	//that we can pipe into netcat to guest machine
	string line;
	int currentLine = 0;
	int lineIndex = 0;
	size_t linePos = 0;
	...
	for (int y = height-1; y >= 0; y--) {
		for (int x = 0; x < width; x++) {	
			...
			line.push_back((char)dataBMP[width*y+x]);
		}
	}

	cout << "Trying to create OFS formatted file." << endl;
	ofstream newFile("temp.ofs");
	...
	if (newFile.is_open()) {
		...
		cerr << "Couldn't open temp file for holding formatted osakaOS data." << endl; 
	}
