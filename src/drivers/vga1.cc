...
void VideoGraphicsArray::MakeWave(int32_t x, int32_t y, 
				  ...
				  //uint8_t amp, uint8_t freq, float waveInc) {

	/*
	uint32_t pixelsOffset = WIDTH_13H*y+x;

	for (int i = 0; i < w; i++) {
	
		uint8_t bufStrip[h];
		int16_t indexAmp = amp*((int16_t)sin(((float)i/freq) + (float)waveInc));
		...
		if (indexAmp != 0) {	
			...
			for (int j = 0; j < h; j++) { 
				...
				uint32_t pixelsIndex = pixelsOffset+(w*j+i);

				if (pixelsIndex >= 0 && pixelsIndex < BUFFER_SIZE_13H) {
			
					bufStrip[j] = this->pixels[pixelsIndex]; 
					this->pixels[pixelsOffset+(w*j+i)] = W_EMPTY;
				}
			}
			
			for (int j = 0; j < h; j++) {
				
				uint32_t pixelsIndex = (WIDTH_13H*(y+j)+(x+i));
			
				if (pixelsIndex >= 0 && pixelsIndex < BUFFER_SIZE_13H) {
				
					this->pixels[WIDTH_13H*(y+j)+(x+i)] = bufStrip[j];
				}
			}
		}
	}
